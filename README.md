Forking for possible future development practice, if I can get a second gopro to test on. My goals would be trying to allow multiple gopro interfacing, and an option for a safe exit of the program while in autostart function (I encountered a bug where when exiting, the gopro will continue thinking it's still in webcam mode. Running the program a second time will get stuck, with this error on exit: `udp://@0.0.0.0:8554?overrun_nonfatal=1&fifo_size=50000000: Immediate exit requested`, however, tricking the gopro out of webcam mode. Running the program a third time after this, the gopro will start working as normal).
